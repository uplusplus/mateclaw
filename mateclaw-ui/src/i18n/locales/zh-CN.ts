export default {
  ...
  settings: {
    ...
    model: {
      ...
      manageTitle: '管理模型',
      addModel: '新增模型',
      saveInputWindow: '保存窗口',
      savingInputWindow: '保存中...',
      inputWindowPlaceholder: '系统默认值会预填到这里',
      inputWindowHint: '这里会先带入系统默认输入窗口；保存后写入当前模型，聊天与运行时以模型自己的设置值为准。',
      inputWindowConfigured: '输入窗口：{count} tokens',
      inputWindowGlobalDefault: '输入窗口：未设置，预填默认 {count} tokens',
      inputWindowSaved: '模型输入窗口已保存',
      inputWindowSaveFailed: '模型输入窗口保存失败',
      inputWindowInvalid: '输入窗口必须是大于 0 的整数',
      inputWindowUnsupported: '当前模型缺少可编辑配置 ID，暂时无法保存输入窗口',
      embedding: {
        title: 'Embedding 模型',
      ...
      modelParamsLoading: '正在加载模型参数...',
      saveModelParams: '保存参数',
      savingModelParams: '保存中...',
      modelParamsSaved: '模型参数已保存',
      modelParamsSaveFailed: '模型参数保存失败',
      modelParamsUnsupported: '该模型暂不支持编辑参数',
      modelParamsNameRequired: '模型显示名称不能为空',
      modelParamsNumberInvalid: '{field} 必须是数字',
      modelParamsIntegerInvalid: '{field} 必须是大于 0 的整数',
    ...
    fields: {
      ...
      streamEnabled: '流式响应',
      debugMode: '调试模式',
      defaultMaxInputTokens: '系统默认输入窗口',
      searchEnabled: '启用搜索',
    ...
    hints: {
      ...
      defaultMaxInputTokens: '只在设置页用于预填模型输入窗口；保存后仍以各模型自己的输入窗口配置为准。',
  ...
  skills: {
    ...
    import: {
      ...
      sourceLabel: '来源',
      sameNameFlag: '同名候选',
      sameNameWarning: '发现同名 skill，请优先核对作者、slug、统计数据和来源地址',
      statusPending: '等待中',
      zipTab: 'ZIP 导入',
